...
func (pn *ProcNodes) run(bestTour *int, tnumRows int, mutex *sync.Mutex, c *mycost.Cost) {
    x := pn.threadNumber
    for !pn.stop  && pn.queue.Len() > 0 {    
        ...
        if next.Size() == tnumRows - 1 && next.LowerBound() < *bestTour {           
            mutex.Lock()
    ...
    if !pn.stop {                            // while loop finished due to   QUEUE.SIZE() == 0
        mutex.Lock()
        pn.tsp.Stop(false, pn.threadNumber)
        mutex.Unlock()
    }
}
